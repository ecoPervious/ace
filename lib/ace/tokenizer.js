    ...
    **/
    this.getLineTokens = function(line, startState, row) {
        ...
        while (match = re.exec(line)) {
            ...
            var rule = null;
            var value = [match[0]];
            var dyn_next = false; // ugly hack to control next state dynamically
            ...
            for (var i = 0; i < match.length-2; i++) {
                ...
                if (typeof rule.token == "function") {
                    type = rule.token.apply(this, value.concat([row]));
                ...
                if (rule.next || dyn_next) {
                    currentState = dyn_next ? dyn_next : rule.next;
